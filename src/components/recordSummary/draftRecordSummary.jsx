...
const DraftRecordSummary = (props) => {
  const { records } = props;
  const filteredRecords = records.filter(record => record.status === 'draft');
  return filteredRecords.map(data => (
...
export default connect(mapStateToProps)(DraftRecordSummary);
